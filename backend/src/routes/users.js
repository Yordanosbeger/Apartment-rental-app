...
module.exports = (db) => {
  ...
  router.post('/login', async (req, res) => {
    ...
    try {
      ...
      if (user.role === 'tenant') {
        const tenantResult = await db.query(`SELECT * FROM tenant_profiles WHERE user_id = $1;`, [user.id]);
        profileData = tenantResult.rows[0];
      } else if (user.role === 'landlord') {
        const landlordResult = await db.query(`SELECT * FROM landlord_profiles WHERE user_id = $1;`, [user.id]);
  ...
  router.post('/register', async (req, res) => {
    ...
    try {
      ...
      if (normalizedRole === 'tenant') {
        await db.query(
          `INSERT INTO tenant_profiles (user_id, firstname, lastname) 
           VALUES ($1, $2, $3);`,
          [userId, firstName, lastName] // Defaults for `pet_friendly` and `verification_status`
        );
      } else if (normalizedRole === 'landlord') {
        await db.query(
          `INSERT INTO landlord_profiles (user_id, firstname, lastname) 
           VALUES ($1, $2, $3);`,
          [userId, firstName, lastName] // Defaults for `pet_friendly` and `verification_status`
        );
      }
